...
// runs, so the advertised limit has to stay under that when deployed there.
const VERCEL_BODY_LIMIT_BYTES = 4 * MEGABYTE;
...
function resolveUploadLimit(env) {
  ...
  return env.VERCEL ? Math.min(requested, VERCEL_BODY_LIMIT_BYTES) : requested;
}
